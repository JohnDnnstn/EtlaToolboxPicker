...
using EtlaToolboxPicker.EtlaToolbelt.Contexts;
...
/// See <see cref="ContextImpl"/> for the default implementation
/// </summary>
public abstract class Context : Fascia<IContext, ContextImpl>
{
    ...
    /// <summary>The name of the company that owns the application.  Used when logging to a file sink.</summary>
    public static string Company { get => Implementation.Company; set => Implementation.Company = value; }
    ...
    /// <summary>Attempts to report if the application is being debugged</summary>
    /// <returns><c>true</c>if the application is being debugged; <c>false</c> otherwise</returns>
    public static bool IsBeingDebugged() => Implementation.IsBeingDebugged();

    /// <summary>The GUID asociated with the Assembly (as a string)</summary>
    public static string? AppGuidString => Assembly.GetExecutingAssembly()?.GetCustomAttribute<GuidAttribute>()?.Value;

    /// <summary>The GUID asociated with the Assembly</summary>
    public static Guid? AppGuid { get; set; } = AppGuidString == null ? null : new Guid(AppGuidString);
}
